...
function loadConfig(workspaceDir: string) {
  // 配置文件查找顺序
  const configPaths = [
    join(workspaceDir, '.deepbot', 'tools', 'my-tool', 'config.json'),
    join(homedir(), '.deepbot', 'tools', 'my-tool', 'config.json'),
  ];
...
async function loadDependency(): Promise<any> {
  const toolDir = join(homedir(), '.deepbot', 'tools', 'my-tool');
...
- 检查依赖是否已安装到 `~/.deepbot/tools/<tool-name>/node_modules/`
- 提供安装命令提示
