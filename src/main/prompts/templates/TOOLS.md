...
### 核心原则
1. **异步双向对话**：Tab 之间通过发送消息进行协作
2. **立即返回**：发送消息后立即返回，不等待结果
3. **自动排队**：如果目标 Tab 正在处理任务，消息会自动排队
4. **主动回复**：目标 Tab 处理完成后，可以主动发送消息回复
...
2. 工具立即返回成功（不等待 B 的回复）
3. 消息发送到 Agent B 的 Tab（标记来源"[来自 Agent A]"）
4. 如果 Agent B 正在处理任务，消息会自动排队
5. Agent B 处理完消息后，可以使用 cross_tab_call 发送回复给 Agent A
6. Agent A 收到回复消息（标记来源"[来自 Agent B]"）
```
