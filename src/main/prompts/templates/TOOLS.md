...
### web_fetch
**用途**: 从 URL 获取网页内容并提取主要内容，转换为 Markdown 格式

**核心功能**:
- 使用 Readability 算法提取主要内容（去除广告、导航等）
- HTML → Markdown 转换（便于阅读和处理）
- SSRF 防护（防止访问内网地址）
- 大小限制和超时控制

**参数**:
- `url`: HTTP 或 HTTPS URL（必填）
- `mode`: 获取模式（可选，默认 `truncated`）
  - `full`: 完整内容（最多 10MB）
  - `truncated`: 截断内容（前 8KB，适合快速预览）
  - `selective`: 搜索特定内容（需要提供 searchPhrase）
- `searchPhrase`: 搜索短语（仅在 `selective` 模式下需要）
...
**⚠️ 使用建议**:
- 适合获取文章、文档、博客等静态内容
- 不适合需要 JavaScript 渲染的动态页面（使用 `browser` 工具）
- 不适合需要登录或交互的页面（使用 `browser` 工具）
- 优先使用 `truncated` 模式快速预览，需要时再用 `full` 模式

**web_fetch vs browser**:
- `web_fetch`: 轻量级，快速，适合静态内容
- `browser`: 重量级，慢，适合动态页面和交互操作
...
### chat
**用途**: 调用 AI 模型进行对话、翻译、总结、改写等任务，支持长文本自动分段处理

**核心功能**:
- 调用配置的 AI 模型进行对话
- 自动检测文本长度并分段处理（每段最多 8000 字符）
- 支持自定义系统提示词
- 自动合并分段结果

**参数**:
- `prompt`: 用户提示词或问题（必填）
- `content`: 需要处理的内容（可选，如需要翻译的文本、需要总结的文章等）
- `systemPrompt`: 系统提示词（可选，用于指定 AI 的角色和行为）
- `maxChunkSize`: 分段大小（可选，默认 8000 字符）
...
```json
{
  "prompt": "将以下英文翻译成中文，保持格式和术语准确",
  "content": "Long English text here..."
}
```
...
```json
{
  ...
  "systemPrompt": "你是一位专业的技术文档编辑，擅长将口语化的内容改写为专业的技术文档"
}
```
...
**⚠️ 使用建议**:
- 适合翻译、总结、改写、问答等任务
- 自动处理长文本（超过 8000 字符会自动分段）
- 分段处理时会保持上下文连贯性
- 使用系统配置中的 AI 模型（与主对话模型相同）

**chat vs web_search**:
- `chat`: 通用 AI 对话，适合翻译、总结、改写等任务
- `web_search`: 网络搜索，适合获取最新信息、新闻、天气等
