...
export const api = {
  ...
  async login(password: string): Promise<{ token: string }> {
    if (isElectron()) {
      throw new Error('Electron 模式不需要登录');
    }
  ...
  async getConfig(): Promise<any> {
    if (isElectron()) {
      return (window as any).electron.ipcRenderer.invoke('get-config');
    }
  ...
  async updateConfig(updates: any): Promise<void> {
    if (isElectron()) {
      return (window as any).electron.ipcRenderer.invoke('update-config', updates);
    }
  ...
  async getModelConfig(): Promise<any> {
    if (isElectron()) {
      return (window as any).electron.ipcRenderer.invoke('model-config:get');
    }
    // Web 模式：从通用配置中提取模型配置
  ...
  async saveModelConfig(config: any): Promise<any> {
    if (isElectron()) {
      return (window as any).electron.ipcRenderer.invoke('model-config:save', { config });
    }
    // Web 模式：通过 updateConfig 保存模型配置
  ...
  async getTabAgentName(tabId: string): Promise<{ success: boolean; agentName: string; userName: string; error?: string }> {
    if (isElectron()) {
      return (window as any).deepbot.getTabAgentName(tabId);
    }
    // Web 模式：从配置中获取名称
    const config = await webClient.getConfig();
    return { 
      success: true, 
      agentName: config.names?.agentName || 'DeepBot', 
      userName: config.names?.userName || '用户' 
    };
  },
  ...
  async checkEnvironment(action: 'check' | 'get_status'): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.checkEnvironment(action);
    }
    // Web 模式：调用后端 API
  ...
  async getImageGenerationToolConfig(): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.getImageGenerationToolConfig();
    }
    // Web 模式：从通用配置中提取图片生成工具配置
    const config = await webClient.getConfig();
  ...
  async saveImageGenerationToolConfig(config: any): Promise<void> {
    if (isElectron()) {
      return (window as any).deepbot.saveImageGenerationToolConfig(config);
    }
    // Web 模式：通过 updateConfig 保存图片生成工具配置
    await webClient.updateConfig({ imageGeneration: config });
  },
  
  async getWebSearchToolConfig(): Promise<any> {
    if (isElectron()) {
      return (window as any).electron.ipcRenderer.invoke('tool-config:web-search:get');
    }
    // Web 模式：从通用配置中提取 Web Search 工具配置
    const config = await webClient.getConfig();
    return {
      success: true,
  ...
  async saveWebSearchToolConfig(config: any): Promise<any> {
    if (isElectron()) {
      return (window as any).electron.ipcRenderer.invoke('tool-config:web-search:save', { config });
    }
    // Web 模式：通过 updateConfig 保存 Web Search 工具配置
  ...
  async getWorkspaceSettings(): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.getWorkspaceSettings();
    }
    // Web 模式：从通用配置中提取工作目录配置
    const config = await webClient.getConfig();
    return { 
      success: true, 
  ...
  async getDefaultWorkspaceSettings(): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.getDefaultWorkspaceSettings();
    }
    // Web 模式：返回默认配置（从环境变量或硬编码）
    return { 
      success: true, 
      settings: {
        workspaceDir: '~/',
        scriptDir: '~/.deepbot/scripts',
        skillDirs: ['~/.agents/skills'],
        defaultSkillDir: '~/.agents/skills',
        imageDir: '~/.deepbot/generated-images',
        memoryDir: '~/.deepbot/memory',
        sessionDir: '~/.deepbot/sessions',
      }
    };
  },
  ...
  async saveWorkspaceSettings(settings: any): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.saveWorkspaceSettings(settings);
    }
    // Web 模式：通过 updateConfig 保存工作目录配置
  ...
  async addSkillDir(dir: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.addSkillDir(dir);
    }
    // Web 模式：通过修改 workspace.skillDirs 实现
    const config = await webClient.getConfig();
  ...
  async removeSkillDir(dir: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.removeSkillDir(dir);
    }
    // Web 模式：通过修改 workspace.skillDirs 实现
    const config = await webClient.getConfig();
    const currentSettings = config.workspace || {};
  ...
  async setDefaultSkillDir(dir: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.setDefaultSkillDir(dir);
    }
    // Web 模式：通过修改 workspace.defaultSkillDir 实现
    const config = await webClient.getConfig();
    const currentSettings = config.workspace || {};
  ...
  async launchChromeWithDebug(port: number): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.launchChromeWithDebug(port);
    }
    // Web 模式：调用后端 API
  ...
  async connectorGetAll(): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorGetAll();
    }
    // Web 模式：调用后端 API
  ...
  async connectorGetConfig(connectorId: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorGetConfig(connectorId);
    }
    // Web 模式：调用后端 API
  ...
  async connectorSaveConfig(connectorId: string, config: any): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorSaveConfig(connectorId, config);
    }
    // Web 模式：调用后端 API
  ...
  async connectorStart(connectorId: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorStart(connectorId);
    }
    // Web 模式：调用后端 API
  ...
  async connectorStop(connectorId: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorStop(connectorId);
    }
    // Web 模式：调用后端 API
  ...
  async connectorHealthCheck(connectorId: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorHealthCheck(connectorId);
    }
    // Web 模式：调用后端 API
  ...
  async connectorApprovePairing(pairingCode: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorApprovePairing(pairingCode);
    }
    // Web 模式：调用后端 API
  ...
  async connectorSetAdminPairing(connectorId: string, userId: string, isAdmin: boolean): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorSetAdminPairing(connectorId, userId, isAdmin);
    }
    // Web 模式：调用后端 API
  ...
  async connectorDeletePairing(connectorId: string, userId: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorDeletePairing(connectorId, userId);
    }
    // Web 模式：调用后端 API
  ...
  async scheduledTask(request: any): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.scheduledTask(request);
    }
    // Web 模式：调用后端 API
  ...
  async getAllTabs(): Promise<{ success: boolean; tabs?: AgentTab[]; error?: string }> {
    if (isElectron()) {
      return (window as any).deepbot.getAllTabs();
    }
  ...
  async createTab(title?: string): Promise<{ success: boolean; tab?: AgentTab; error?: string }> {
    if (isElectron()) {
      return (window as any).deepbot.createTab(title);
    }
  ...
  async closeTab(tabId: string): Promise<{ success: boolean; error?: string }> {
    if (isElectron()) {
      return (window as any).deepbot.closeTab(tabId);
    }
  ...
  async sendMessage(content: string, sessionId?: string): Promise<void> {
    if (isElectron()) {
      return (window as any).deepbot.sendMessage(content, sessionId);
    }
  ...
  async stopGeneration(sessionId?: string): Promise<void> {
    if (isElectron()) {
      return (window as any).deepbot.stopGeneration(sessionId);
    }
    // Web 模式：调用后端 API
  ...
  async connectorGetPairingRecords(): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.connectorGetPairingRecords();
    }
    // Web 模式：调用后端 API
  ...
  async uploadFile(fileName: string, dataUrl: string, fileSize: number, fileType: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.uploadFile(fileName, dataUrl, fileSize, fileType);
    }
    // Web 模式：调用后端 API
  ...
  async uploadImage(fileName: string, dataUrl: string, fileSize: number): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.uploadImage(fileName, dataUrl, fileSize);
    }
    // Web 模式：调用后端 API
  ...
  async readImage(filePath: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.readImage(filePath);
    }
    // Web 模式：调用后端 API
  ...
  async deleteTempFile(filePath: string): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.deleteTempFile(filePath);
    }
    // Web 模式：调用后端 API
  ...
  async skillManager(request: any): Promise<any> {
    if (isElectron()) {
      return (window as any).deepbot.skillManager(request);
    }
    // Web 模式：调用后端 API
  ...
  onTabCreated(callback: (data: { tab: AgentTab }) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onTabCreated(callback);
    }
  ...
  onTabUpdated(callback: (data: { tabId: string; title: string }) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onTabUpdated(callback);
    }
  ...
  onTabMessagesCleared(callback: (data: { tabId: string }) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onTabMessagesCleared(callback);
    }
  ...
  onTabHistoryLoaded(callback: (data: { tabId: string; messages: Message[] }) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onTabHistoryLoaded(callback);
    }
  ...
  onNameConfigUpdate(callback: (config: any) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onNameConfigUpdate(callback);
    }
  ...
  onModelConfigUpdate(callback: () => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onModelConfigUpdate(callback);
    }
  ...
  onPendingCountUpdate(callback: (data: { pendingCount: number }) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onPendingCountUpdate(callback);
    }
  ...
  onClearAllMessages(callback: () => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onClearAllMessages(callback);
    }
  ...
  onClearChat(callback: (data: { sessionId: string }) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onClearChat?.(callback) || (() => {});
    }
  ...
  onMessageStream(callback: (chunk: any) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onMessageStream(callback);
    }
  ...
  onExecutionStepUpdate(callback: (data: any) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onExecutionStepUpdate?.(callback) || (() => {});
    }
  ...
  onMessageError(callback: (error: any) => void): () => void {
    if (isElectron()) {
      return (window as any).deepbot.onMessageError(callback);
    }
  ...
  createWebSocket(): WebSocket | null {
    ...
    // WebSocket 连接建立后，自动订阅 default Tab
    ws.addEventListener('open', () => {
      console.log('[WebSocket] 连接已建立，订阅 default Tab');
  ...
  _registerWebEvent(eventType: string, callback: Function): () => void {
    ...
    return () => {
      ...
      if (listeners.size === 0) {
        webEventListeners.delete(eventType);
      }
    };
  },
};
