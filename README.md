...
```bash
# 克隆仓库
git clone https://github.com/yourusername/slhbot.git
cd slhbot
...
macOS 首次打开 史丽慧小助理 时可能会提示安全警告，选择对应的解决方法：
...
史丽慧小助理 采用模块化架构，支持多 Agent 互相对话和协作：
...
史丽慧小助理 支持通过 Connector 系统接入外部平台，实现跨平台交互。

### 已支持平台

#### 飞书 (Feishu)

通过飞书机器人与 史丽慧小助理 交互，支持私聊和群聊。
...
史丽慧小助理 支持创建自定义工具来扩展功能。所有工具都是内置工具，代码位于 `src/main/tools/` 目录。
...
2. 用户级别：`~/.slhbot/tools/email-tool/config.json`
...
- **配置文件**: 从 `~/.slhbot/tools/<tool-name>/config.json` 读取配置
...
史丽慧小助理 实现了严格的安全限制，确保 AI Agent 只能访问用户明确授权的目录：
...
| **脚本目录** | `~/.slhbot/scripts` | Python 脚本存储 | ✅ |
| **Skill 目录** | `~/.agents/skills` | Skill 包安装 | ✅ |
| **图片目录** | `~/.slhbot/generated-images` | AI 生成图片保存 | ✅ |
...
史丽慧小助理 支持强大的长期记忆功能，可以记住用户的偏好和重要信息。

### 全局记忆

- **存储位置**: `~/.slhbot/memory/MEMORY.md`
...
```
用户: "创建一个销售分析 Agent"
史丽慧小助理: "已创建新 Tab，这个 Agent 将专注于客户关系管理和销售数据分析"
...
将 Skill 目录放到 `~/.agents/skills/` 下，重启 史丽慧小助理 即可自动加载。
...
史丽慧小助理 针对标准对话模型进行了优化。带有内置思考/推理模式的模型（如通义千问的 QwQ 系列、OpenAI 的 o1 系列，或其他具有显式推理步骤的模型）可能会导致：
...
- **作者**: K罗@格灵深瞳
- **问题反馈**: [GitHub Issues](https://github.com/yourusername/slhbot/issues)
