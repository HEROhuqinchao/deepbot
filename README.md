...
**新增功能**：
- ✨ **飞书集成**：支持通过飞书机器人与 DeepBot 交互，支持私聊和群聊，独立会话管理
- ✨ **Web Fetch Tool**：新增网页内容获取工具，可提取网页正文、下载网页数据
- ✨ **Chat Tool**：新增 AI 对话处理工具，用于工具内部调用 AI，不占用主 Agent 上下文

**优化改进**：
- 🔧 流式输出优化：Chat Tool 支持实时显示处理进度
- 🔧 AutoContinue 改进：保留执行步骤历史，提供完整的任务执行记录
- 🔧 前端体验优化：修复执行步骤显示时序问题，确保工具调用进度实时可见
