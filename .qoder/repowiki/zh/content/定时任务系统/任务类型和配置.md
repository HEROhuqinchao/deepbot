...
## 简介
本文件系统化阐述 史丽慧小助理 的定时任务类型与配置体系，围绕 ScheduledTask 接口与 TaskSchedule 类型定义，详解一次性任务、周期性任务与 Cron 任务的配置参数、使用场景与最佳实践；解释调度参数的语义与取值范围；给出任务配置验证规则、版本兼容与迁移策略，并提供可视化流程图帮助理解。
...
## 结论
史丽慧小助理 的定时任务系统以清晰的类型定义为基础，结合 SQLite 持久化、每秒轮询调度与专用 Tab 执行，提供了稳定可靠的自动化执行能力。通过自然语言解析与严格的参数校验，降低了用户配置门槛；通过 maxRuns、禁用与历史记录等机制，保障了可控性与可观测性。
