...
## 简介
本文件面向 DeepBot 记忆管理工具，系统阐述其全局记忆存储、独立记忆管理与记忆更新机制，涵盖 API 接口、数据结构、存储策略、持久化与恢复、以及最佳实践与性能优化建议。读者可据此实现稳定、可扩展的记忆管理能力，并在多会话/多标签页场景中保持一致的上下文体验。
