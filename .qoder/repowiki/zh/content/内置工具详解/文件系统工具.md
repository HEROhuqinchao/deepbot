...
## 简介
本文件系统工具为 DeepBot 提供安全可控的文件读写与编辑能力，并通过“白名单目录”机制限制文件访问范围，确保仅能在受信任的工作空间内进行操作。工具链覆盖：
...
- 白名单目录来源
  - 工作空间目录 workspaceDir
  - 数据库目录（Docker 模式：/data/db；Electron 模式：~/.deepbot）
  - 脚本目录 scriptDir
  - 技能目录 skillDirs[]
  - 图片生成目录 imageDir
  - 记忆目录 memoryDir
...
- 路由
  ...
  - DELETE /temp：删除临时文件（仅允许删除工作空间下的 .deepbot/temp/uploads 子树）
- 适配器
...
- 删除临时文件失败
  - 现象：返回错误信息
  - 处理：确认路径在工作空间下的 .deepbot/temp/uploads 子树内
...
## 结论
DeepBot 文件系统工具通过“白名单目录 + 路径断言 + 参数兼容 + 结果优化”的设计，在保障安全性的同时提供了简洁易用的文件操作能力。工具链覆盖从 Agent 工具到 Web 上传/读取/删除的完整场景，并在 Docker 与 Electron 两种运行模式下均能稳定工作。
...
- DELETE /files/temp
  - 查询参数：path
  - 限制：仅允许删除工作空间下的 .deepbot/temp/uploads 子树
