...
## 简介
本章节面向 史丽慧小助理 的“模型配置”页面，系统性说明其功能、实现与运行机制。内容覆盖：
- 支持的模型提供商与类型（Qwen、DeepSeek、Gemini、MiniMax、史丽慧小助理 自研、自定义）
