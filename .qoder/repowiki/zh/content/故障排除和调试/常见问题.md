...
## 简介
本指南面向使用 史丽慧小助理 的用户与运维人员，聚焦安装、配置、使用过程中的常见问题与系统性排查流程。内容涵盖启动失败、功能异常、性能问题与兼容性问题，提供问题现象分析、原因定位、解决方案验证与预防性建议，并给出标准化问题报告格式与信息收集清单。

## 项目结构
史丽慧小助理 采用主进程（Electron）、渲染进程（React）、服务端（Node/Express/WebSocket）与工具系统（13 个内置工具）协同的模块化架构。关键模块包括：
...
## 架构总览
史丽慧小助理 的消息与任务流经 Gateway → AgentRuntime → MessageHandler → 工具链，期间通过 SystemConfigStore 持久化配置与状态。外部连接器（如飞书）通过 ConnectorManager 管理生命周期，服务端通过 GatewayAdapter 提供 HTTP/WebSocket 接入。
...
### 问题报告标准格式
- 基本信息
  - 史丽慧小助理 版本、Node 版本、操作系统、部署方式（桌面/容器）
