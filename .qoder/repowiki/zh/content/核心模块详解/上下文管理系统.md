...
## 简介
本文件面向 DeepBot 的上下文管理系统，系统性阐述上下文管理器的设计理念、历史修剪机制、工具结果修剪策略，并深入解析 ContextManager 类的实现细节（上下文状态维护、消息历史管理、内存优化策略）。文档还记录历史修剪器与工具结果修剪器的工作原理（修剪规则、阈值配置、清理策略），并提供配置与使用示例、与 Agent Runtime 的集成方式、性能影响评估以及优化建议与最佳实践。
